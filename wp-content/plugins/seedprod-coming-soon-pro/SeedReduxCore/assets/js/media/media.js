/*global seedredux_change, wp, seedredux, libFilter */
...
(function($){
    ...
    seedredux.field_objects.media   = seedredux.field_objects.media || {};
    ...
    seedredux.field_objects.media.init = function( selector ) {
        if ( !selector ) {
            selector = $( document ).find( ".seedredux-group-tab:visible" ).find( '.seedredux-container-media:visible' );
        }
        $( selector ).each(
            function() {
                var el = $( this );
                ...
                el.find( '.remove-image, .remove-file' ).unbind( 'click' ).on(
                    'click', function() {
                        seedredux.field_objects.media.removeFile( $( this ).parents( 'fieldset.seedredux-field:first' ) );
                    }
                );
                // Upload media button
                el.find( '.media_upload_button' ).unbind().on(
                    'click', function( event ) {
                        seedredux.field_objects.media.addFile( event, $( this ).parents( 'fieldset.seedredux-field:first' ) );
                    }
                );
            }
        );
    ...
    seedredux.field_objects.media.addFile = function( event, selector ) {
        ...
        frame.on(
            'select', function() {
                ...
                if ( typeof seedredux.field_objects.media === 'undefined' || typeof seedredux.field_objects.media === undefined ) {
                    seedredux.field_objects.media = {};
                }
                ...
                selector.find( '.seedredux-background-properties' ).slideDown();
            }
        );
    ...
    seedredux.field_objects.media.removeFile = function( selector ) {
        ...
        seedredux_change( $( selector ).find( '.upload-id' ) );
        selector.find( '.seedredux-background-properties' ).hide();
