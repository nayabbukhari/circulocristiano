<?php
     ...
     * @subpackage  Core
     * @author      SeedRedux Framework Team
     */
...
// Fix for the GT3 page builder: http://www.gt3themes.com/wordpress-gt3-page-builder-plugin/
    ...
    if ( ! class_exists( 'SeedReduxFrameworkInstances' ) ) {
        // Instance Container
        require_once( dirname( __FILE__ ) . '/inc/class.seedredux_instances.php' );
        require_once( dirname( __FILE__ ) . '/inc/lib.seedredux_instances.php' );
    }

    if ( class_exists( 'SeedReduxFrameworkInstances' ) ) {
        add_action( 'seedredux/init', 'SeedReduxFrameworkInstances::get_instance' );
    }
...
// Don't duplicate me!
    if ( ! class_exists( 'SeedReduxFramework' ) ) {
        ...
        // SeedRedux API class  :)
        require_once( dirname( __FILE__ ) . '/inc/class.seedredux_api.php' );

        // General helper functions
        require_once( dirname( __FILE__ ) . '/inc/class.seedredux_helpers.php' );

        // General functions
        require_once( dirname( __FILE__ ) . '/inc/class.seedredux_functions.php' );
        require_once( dirname( __FILE__ ) . '/inc/class.p.php' );

        require_once( dirname( __FILE__ ) . '/inc/class.seedredux_filesystem.php' );

        // ThemeCheck checks
        require_once( dirname( __FILE__ ) . '/inc/class.seedredux_themecheck.php' );

        // Welcome
        require_once( dirname( __FILE__ ) . '/inc/welcome/welcome.php' );

        //require_once( dirname( __FILE__ ) . '/inc/class.seedredux_sass.php' );

        /**
         * Main SeedReduxFramework class
         *
         * @since       1.0.0
         */
        class SeedReduxFramework {
            ...
            public static function init() {
                ...
                if ( self::$_is_plugin == true || self::$_as_plugin == true ) {
                    ...
                    } else if ( strpos( SeedRedux_Helpers::cleanFilePath( __FILE__ ), SeedRedux_Helpers::cleanFilePath( get_stylesheet_directory() ) ) !== false ) {
            ...
            public $framework_url = 'http://www.seedreduxframework.com/';
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( ! empty ( $this->args['opt_name'] ) ) {
                     ...
                     */
                    $this->sections = apply_filters( 'seedredux-sections', $sections ); // REMOVE LATER
                    /**
                     ...
                     * @param  array $sections field option sections
                     */
                    $this->sections = apply_filters( "seedredux-sections-{$this->args['opt_name']}", $this->sections ); // REMOVE LATER
                    /**
                    ...
                    if ( $this->args['database'] == "network" && $this->args['network_admin'] ) {
                        add_action( 'network_admin_edit_seedredux_' . $this->args['opt_name'], array(
                            $this,
                            'save_network_page'
                        ), 10, 0 );
                    ...
                    if ( $this->args['dev_mode'] == true || SeedRedux_Helpers::isLocalHost() === true ) {
                        if ( ! isset ( $GLOBALS['seedredux_notice_check'] ) ) {
                            ...
                            $params = array(
                                'dir_name'    => 'notice',
                                'server_file' => 'http://www.seedreduxframework.com/' . 'wp-content/uploads/seedredux/seedredux_notice.json',
                                'interval'    => 3,
                                'cookie_id'   => 'seedredux_blast',
                            );
...
// __construct()
            ...
            private function set_default_args() {
                $this->args = array(
                    ...
                    'class'                     => '',
                    // Class that gets appended to all seedredux-containers
                    'admin_bar'                 => true,
                    ...
                    'templates_path'            => '',
                    // Path to the templates file for various SeedRedux elements
                    'ajax_save'                 => true,
            ...
            public function vc_fixes() {
                if ( seedredux_helpers::isFieldInUse( $this, 'ace_editor' ) ) {
            ...
            public function network_admin_bar( $wp_admin_bar ) {
                ...
                $args = array(
                    ...
                    'meta'   => array( 'class' => 'seedredux-network-admin' )
                );
            ...
            public function _update_check() {
                ...
                if ( ! isset ( $GLOBALS['seedredux_update_check'] ) ) {
                    SeedRedux_Functions::updateCheck( self::$_version );
                    $GLOBALS['seedredux_update_check'] = 1;
                }
            }

            public function _admin_notices() {
                SeedRedux_Functions::adminNotices( $this->admin_notices );
            }
            ...
            private function _internationalization() {
                 ...
                 * filter 'seedredux/textdomain/{opt_name}'
                 *
                 * @param string     The locale of the blog or from the 'locale' hook
                 * @param string     'seedredux-framework'  text domain
                 */
                $locale = apply_filters( "seedredux/textdomain/{$this->args['opt_name']}", get_locale(), 'seedredux-framework' );
                ...
                load_textdomain( 'seedredux-framework', self::$_dir . 'languages/' . $locale . '.mo' );
            }
...
// get_instance()
            ...
            private function _tracking() {
                ...
                $tracking = SeedRedux_Tracking::get_instance();
                $tracking->load( $this );
            }
// _tracking()
...
// set()
            ...
            private function set_global_variable() {
                if ( $this->args['global_variable'] ) {
                    ...
                    /**
                     * filter 'seedredux/options/{opt_name}/global_variable'
                     *
                     * @param array $value option value to set global_variable with
                     */
                    $GLOBALS[ $this->args['global_variable'] ] = apply_filters( "seedredux/options/{$this->args['opt_name']}/global_variable", $this->options );
...
// set_global_variable()
            ...
            public function set_options( $value = '' ) {
                ...
                if ( ! empty ( $value ) ) {
                    ...
                    //do_action( "seedredux-saved-{$this->args['opt_name']}", $value ); // REMOVE
                    //do_action( "seedredux/options/{$this->args['opt_name']}/saved", $value, $this->transients['changed_values'] );
                }
            }
// set_options()
            ...
            public function get_options() {
                ...
                /**
                 * action 'seedredux/options/{opt_name}/options'
                 *
                 * @param mixed $value option values
                 */
                $this->options = apply_filters( "seedredux/options/{$this->args['opt_name']}/options", $this->options );
...
//return $data;
                /**
                 * filter 'seedredux/options/{opt_name}/wordpress_data/{type}/'
                 *
                 * @deprecated
                 *
                 * @param string $data
                 */
                $data = apply_filters( "seedredux/options/{$this->args['opt_name']}/wordpress_data/$type/", $data ); // REMOVE LATER

                /**
                 * filter 'seedredux/options/{opt_name}/data/{type}'
                 *
                 * @param string $data
                 */
                $data = apply_filters( "seedredux/options/{$this->args['opt_name']}/data/$type", $data );
                ...
                if ( empty ( $data ) && ! empty ( $type ) ) {
                    ...
                    if ( ! empty ( $type ) && empty ( $data ) ) {
                        ...
                        if ( $type == "categories" || $type == "category" ) {
                            ...
                            /**
                             * filter 'seedredux-font-icons'
                             *
                             * @deprecated
                             *
                             * @param array $font_icons array of elusive icon classes
                             */
                            $font_icons = apply_filters( 'seedredux-font-icons', array() ); // REMOVE LATER
                             ...
                             * @param array $font_icons array of elusive icon classes
                             */
                            $font_icons = apply_filters( 'seedredux/font-icons', $font_icons );

                            /**
                             * filter 'seedredux/{opt_name}/field/font/icons'
                             *
                             * @deprecated
                             *
                             * @param array $font_icons array of elusive icon classes
                             */
                            $font_icons = apply_filters( "seedredux/{$this->args['opt_name']}/field/font/icons", $font_icons );
...
// show()
            ...
            public function _default_values() {
                        ...
                        if ( isset ( $section['fields'] ) ) {
                            foreach ( $section['fields'] as $k => $field ) {
                                ...
                                if ( $field['type'] == "section" && isset ( $field['indent'] ) && $field['indent'] == "true" ) {
                                    ...
                                    $field['class'] .= "seedredux-section-indent-start";
                ...
                /**
                 * filter 'seedredux/options/{opt_name}/defaults'
                 *
                ...
                $this->options_defaults             = apply_filters( "seedredux/options/{$this->args['opt_name']}/defaults", $this->options_defaults, $this->transients['changed_values'] );
                ...
                if ( $this->args['menu_type'] == 'hidden' ) {
                    ...
                    if ( true === $this->args['allow_sub_menu'] ) {
                        ...
                        if ( true == $this->args['system_info'] ) {
                            add_submenu_page(
                                $this->args['page_slug'], __( 'System Info', 'seedredux-framework' ), __( 'System Info', 'seedredux-framework' ), $this->args['page_permissions'], $this->args['page_slug'] . '&tab=system_info_default', '__return_null'
                            );
                        }
                    }
                }
            ...
            public function _admin_bar_menu() {
                ...
                if ( $menu ) {
                    ...
                    if ( isset ( $submenu[ $this->args["page_slug"] ] ) && is_array( $submenu[ $this->args["page_slug"] ] ) ) {
                        foreach ( $submenu[ $this->args["page_slug"] ] as $index => $seedredux_options_submenu ) {
                            $subnodeargs = array(
                                'id'     => $this->args["page_slug"] . '_' . $index,
                                'title'  => $seedredux_options_submenu[0],
                                'parent' => $this->args["page_slug"],
                                'href'   => admin_url( 'admin.php?page=' . $seedredux_options_submenu[2] ),
                            );
                    ...
                    $nodeargs = array(
                        ...
                        'title' => $title,
                        // $theme_data->get( 'Name' ) . " " . __( 'Options', 'seedredux-framework-demo' ),
...
// _admin_bar_menu()
            ...
            public function _enqueue_output() {
                        ...
                        foreach ( $section['fields'] as $fieldk => $field ) {
                            if ( isset ( $field['type'] ) && $field['type'] != "callback" ) {
                                ...
                                if ( ! class_exists( $field_class ) ) {
                                     ...
                                     */
                                    $class_file = apply_filters( "seedredux/{$this->args['opt_name']}/field/class/{$field['type']}", self::$_dir . "inc/fields/{$field['type']}/field_{$field['type']}.php", $field );
                                ...
                                if ( ! empty ( $this->options[ $field['id'] ] ) && class_exists( $field_class ) && method_exists( $field_class, 'output' ) && $this->_can_output_css( $field ) ) {
                                    $field = apply_filters( "seedredux/field/{$this->args['opt_name']}/output_css", $field );
                ...
                if ( ! empty ( $this->typography ) && ! empty ( $this->typography ) && filter_var( $this->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
                    ...
                    $typography = new SeedReduxFramework_typography ( null, null, $this );
                    ...
                    <?php
                        ...
                        wp_register_style( 'seedredux-google-fonts-' . $this->args['opt_name'], $protocol . $typography->makeGoogleWebfontLink( $this->typography ), '', $version );
                        wp_enqueue_style( 'seedredux-google-fonts-' . $this->args['opt_name'] );
                    }
                }
            }
// _enqueue_output()
            ...
            public function _enqueue() {
                ...
                $enqueue = new seedreduxCoreEnqueue ( $this );
                $enqueue->init();


//                if ($this->args['sass']['enabled']) {
//                    $ret = seedreduxSassCompiler::compile_sass($this);
//                    
//                    if ($ret == seedreduxSassCompiler::SASS_FILE_COMPILE || $ret == seedreduxSassCompiler::SASS_NO_COMPILE) {
//                        if (file_exists(SeedReduxFramework::$_upload_dir . $this->args['opt_name'] .  '-seedredux.css')) {
//                            wp_enqueue_style(
//                                'seedredux-fields-css', 
//                                SeedReduxFramework::$_upload_url . $this->args['opt_name'] .  '-seedredux.css', 
//                                array(), 
//                                $timestamp, 
//                                'all'
//                            );
...
// _enqueue()
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = __( 'Enable', 'seedredux-framework' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = __( 'Disable', 'seedredux-framework' );
                    }

                    // Make URL
                    $url = '<a class="seedredux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = __( 'moving the mouse over', 'seedredux-framework' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = __( 'clicking', 'seedredux-framework' );
                    }

                    // Construct message
                    $msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'seedredux-framework' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'seedredux-hint-tab',
                        'title'   => __( 'Hints', 'seedredux-framework' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
                ...
                if ( $this->args['help_sidebar'] != '' ) {
                    ...
                    // about hints.
                    if ( true == $this->show_hints ) {
                        $screen->set_help_sidebar( '<p><strong>SeedRedux Framework</strong><br/><br/>Hint Tooltip Preferences</p>' );
                    }
                }

                /**
                 * action 'seedredux-load-page-{opt_name}'
                 *
                 * @deprecated
                 *
                 * @param object $screen WP_Screen
                 */
                do_action( "seedredux-load-page-{$this->args['opt_name']}", $screen ); // REMOVE

                /**
                 * action 'seedredux/page/{opt_name}/load'
                 *
                 * @param object $screen WP_Screen
                 */
                do_action( "seedredux/page/{$this->args['opt_name']}/load", $screen );
            }
// _load_page()

            /**
             * Do action seedredux-admin-head for options page
             *
             * @since       1.0.0
             * @access      public
             * @return      void
            ...
            public function admin_head() {
                /**
                 * action 'seedredux-admin-head-{opt_name}'
                 *
                 * @deprecated
                 *
                 * @param  object $this SeedReduxFramework
                 */
                do_action( "seedredux-admin-head-{$this->args['opt_name']}", $this ); // REMOVE

                /**
                 * action 'seedredux/page/{opt_name}/header'
                 *
                 * @param  object $this SeedReduxFramework
                 */
                do_action( "seedredux/page/{$this->args['opt_name']}/header", $this );
            }
// admin_head()
...
// admin_footer_text()
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = __( 'Default', 'seedredux-framework' ) . ": " . substr( $default_output, 0, - 2 );
                }
...
// get_default_output_string()
            ...
            public function get_header_html( $field ) {
                ...
                if ( isset ( $field['title'] ) && isset ( $field['type'] ) && $field['type'] !== "info" && $field['type'] !== "section" ) {
                    ...
                    if ( isset ( $field['hint'] ) && ! '' == $field['hint'] ) {
                        ...
                        if ( 'true' == $metaVal || empty ( $metaVal ) ) {
                            ...
                            $hint = '<div class="seedredux-hint-qtip" style="float:' . $this->args['hints']['icon_position'] . '; font-size: ' . $size . '; color:' . $hint_color . '; cursor: ' . $pointer . ';" qtip-title="' . $titleParam . '" qtip-content="' . $contentParam . '"><i class="' . $this->args['hints']['icon'] . '"></i>&nbsp&nbsp</div>';
                        }
                    }
                ...
                if ( ! empty ( $th ) ) {
                    $th = '<div class="seedredux_field_th">' . $th . '</div>';
                }
            ...
            public function _register_settings() {
                ...
                foreach ( $this->sections as $k => $section ) {
                    ...
                    /**
                     * filter 'seedredux-section-{index}-modifier-{opt_name}'
                     *
                     * @param array $section section configuration
                     */
                    $section = apply_filters( "seedredux-section-{$k}-modifier-{$this->args['opt_name']}", $section );

                    /**
                     * filter 'seedredux/options/{opt_name}/section/{section.id}'
                     *
                     * @param array $section section configuration
                     */
                    if ( isset ( $section['id'] ) ) {
                        $section = apply_filters( "seedredux/options/{$this->args['opt_name']}/section/{$section['id']}", $section );
                    }
                    ...
                    if ( isset ( $section['fields'] ) ) {
                        foreach ( $section['fields'] as $fieldk => $field ) {
                            ...
                            /**
                             * filter 'seedredux/options/{opt_name}/field/{field.id}'
                             *
                             * @param array $field field config
                             */
                            $field = apply_filters( "seedredux/options/{$this->args['opt_name']}/field/{$field['id']}/register", $field );
                            ...
                            /**
                             * filter 'seedredux-field-{field.id}modifier-{opt_name}'
                             *
                             * @deprecated
                             *
                             * @param array $field field config
                             */
                            $field = apply_filters( "seedredux-field-{$field['id']}modifier-{$this->args['opt_name']}", $field ); // REMOVE LATER
                             ...
                             * @param array $field field config
                             */
                            $field = apply_filters( "seedredux/options/{$this->args['opt_name']}/field/{$field['id']}", $field );
                            ...
                            /**
                             * action 'seedredux/options/{opt_name}/field/field.type}/register'
                             */
                            do_action( "seedredux/options/{$this->args['opt_name']}/field/{$field['type']}/register", $field );
                            ...
                            if ( ! $display || isset ( $this->no_panel_section[ $k ] ) ) {
                                ...
                                if ( isset ( $field['hidden'] ) && $field['hidden'] ) {
                                    $field['label_for'] = 'seedredux_hide_field';
                                }
                ...
                /**
                 * action 'seedredux-register-settings-{opt_name}'
                 *
                 * @deprecated
                 */
                do_action( "seedredux-register-settings-{$this->args['opt_name']}" ); // REMOVE

                /**
                 * action 'seedredux/options/{opt_name}/register'
                 *
                 * @param array option sections
                 */
                do_action( "seedredux/options/{$this->args['opt_name']}/register", $this->sections );
                ...
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
                     ...
                     */
                    do_action( "seedredux-compiler-{$this->args['opt_name']}", $this->options, $this->compilerCSS, $this->transients['changed_values'] ); // REMOVE

                    /**
                     * action 'seedredux/options/{opt_name}a'
                     *
                     * @param array  options
                     * @param string CSS that get sent to the compiler hook
                     */
...
// _register_settings()
            ...
            private function _register_extensions() {
                ...
                /**
                 * action 'seedredux/extensions/{opt_name}/before'
                 *
                 * @param object $this SeedReduxFramework
                 */
                do_action( "seedredux/extensions/{$this->args['opt_name']}/before", $this );
                ...
                foreach ( $folders as $folder ) {
                    ...
                    $extension_class = 'SeedReduxFramework_Extension_' . $folder;

                    /**
                     * filter 'seedredux-extensionclass-load'
                     *
                     * @deprecated
                     *
                     ...
                     */
                    $class_file = apply_filters( "seedredux-extensionclass-load", "$path/$folder/extension_{$folder}.php", $extension_class ); // REMOVE LATER

                    /**
                     * filter 'seedredux/extension/{opt_name}/{folder}'
                     *
                     * @param        string                    extension class file path
                     * @param string $extension_class          extension class name
                     */
                    $class_file = apply_filters( "seedredux/extension/{$this->args['opt_name']}/$folder", "$path/$folder/extension_{$folder}.php", $class_file );
                ...
                /**
                 * action 'seedredux-register-extensions-{opt_name}'
                 *
                 * @deprecated
                 *
                 * @param object $this SeedReduxFramework
                 */
                do_action( "seedredux-register-extensions-{$this->args['opt_name']}", $this ); // REMOVE

                /**
                 * action 'seedredux/extensions/{opt_name}'
                 *
                 * @param object $this SeedReduxFramework
                 */
                do_action( "seedredux/extensions/{$this->args['opt_name']}", $this );
            }
...
//print_r($plugin_options);
                ...
                if ( isset ( $plugin_options['seedredux-no_panel'] ) ) {
                    $keys = explode( '|', $plugin_options['seedredux-no_panel'] );
                    foreach ( $keys as $key ) {
                        $plugin_options[ $key ] = $this->options[ $key ];
                    }
                    if ( isset ( $plugin_options['seedredux-no_panel'] ) ) {
                        unset ( $plugin_options['seedredux-no_panel'] );
                    }
                }
                ...
                if ( ( isset( $plugin_options['import_code'] ) && ! empty( $plugin_options['import_code'] ) ) || ( isset( $plugin_options['import_link'] ) && ! empty( $plugin_options['import_link'] ) ) ) {
                    ...
                    if ( ! empty ( $imported_options ) && is_array( $imported_options ) && isset ( $imported_options['seedredux-backup'] ) && $imported_options['seedredux-backup'] == '1' ) {
                        ...
                        /**
                         * action 'seedredux/options/{opt_name}/import'
                         *
                         * @param  &array [&$plugin_options, seedredux_options]
                         */
                        do_action_ref_array( "seedredux/options/{$this->args['opt_name']}/import", array(
                            &$plugin_options,
                            $imported_options,
                            $this->transients['changed_values']
                        ) );

                        setcookie( 'seedredux_current_tab', '', 1, '/', $time + 1000, "/" );
                        $_COOKIE['seedredux_current_tab'] = 1;
                ...
                if ( ! empty ( $plugin_options['defaults'] ) ) {
                    ...
                    /**
                     * apply_filters 'seedredux/validate/{opt_name}/defaults'
                     *
                     * @param  &array [ $this->options_defaults, $plugin_options]
                     */
                    $plugin_options = apply_filters( "seedredux/validate/{$this->args['opt_name']}/defaults", $this->options_defaults );

                    // Section reset
                    //setcookie('seedredux-compiler-' . $this->args['opt_name'], 1, time() + 3000, '/');
                    ...
                    //setcookie('seedredux-compiler-' . $this->args['opt_name'], 1, time() + 1000, "/");
                    //setcookie("seedredux-saved-{$this->args['opt_name']}", 'defaults', time() + 1000, "/");
                ...
                if ( ! empty ( $plugin_options['defaults-section'] ) ) {
                    if ( isset ( $plugin_options['seedredux-section'] ) && isset ( $this->sections[ $plugin_options['seedredux-section'] ]['fields'] ) ) {
                        /**
                         * apply_filters 'seedredux/validate/{opt_name}/defaults_section'
                         *
                         * @param  &array [ $this->options_defaults, $plugin_options]
                         */
                        foreach ( $this->sections[ $plugin_options['seedredux-section'] ]['fields'] as $field ) {
                        ...
                        $plugin_options = apply_filters( "seedredux/validate/{$this->args['opt_name']}/defaults_section", $plugin_options );
                    }
                    ...
                    //setcookie("seedredux-saved-{$this->args['opt_name']}", 'defaults_section', time() + 1000, "/");
                    unset ( $plugin_options['defaults'], $plugin_options['defaults_section'], $plugin_options['import'], $plugin_options['import_code'], $plugin_options['import_link'], $plugin_options['compiler'], $plugin_options['seedredux-section'] );
...
//                }
                ...
                /**
                 * apply_filters 'seedredux/validate/{opt_name}/before_validation'
                 *
                 * @param  &array [&$plugin_options, seedredux_options]
                 */
                $plugin_options = apply_filters( "seedredux/validate/{$this->args['opt_name']}/before_validation", $plugin_options, $this->options );
                ...
                /**
                 * action 'seedredux-validate-{opt_name}'
                 *
                 * @deprecated
                 *
                 * @param  &array [&$plugin_options, seedredux_options]
                 */
                do_action_ref_array( "seedredux-validate-{$this->args['opt_name']}", array(
                    &$plugin_options,
                    $this->options
                ) ); // REMOVE
                ...
                /**
                 * action 'seedredux/options/{opt_name}/validate'
                 *
                 * @param  &array [&$plugin_options, seedredux_options]
                 */
                do_action_ref_array( "seedredux/options/{$this->args['opt_name']}/validate", array(
                    &$plugin_options,
                    $this->options,
                    $this->transients['changed_values']
                ) );
            ...
            public function ajax_save() {
                ...
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "seedredux_ajax_nonce" ) ) {
                    json_encode( array(
                        'status' => __( 'Invalid security credential, please reload the page and try again.', 'seedredux-framework' ),
                        'action' => 'reload'
                    ) );
                    die();
                }
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $seedredux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        try {
                            ...
                            $success = array(
                                'status'           => 'success',
                                'options'          => $seedredux->options,
                                'errors'           => isset ( $seedredux->localize_data['errors'] ) ? $seedredux->localize_data['errors'] : null,
                                'warnings'         => isset ( $seedredux->localize_data['warnings'] ) ? $seedredux->localize_data['warnings'] : null,
                ...
                foreach ( $sections as $k => $section ) {
                    if ( isset ( $section['fields'] ) ) {
                        foreach ( $section['fields'] as $fkey => $field ) {
                            ...
                            if ( isset ( $field['validate'] ) ) {
                                $validate = 'SeedRedux_Validation_' . $field['validate'];

                                if ( ! class_exists( $validate ) ) {
                                    /**
                                     * filter 'seedredux-validateclass-load'
                                     *
                                     * @deprecated
                                     *
                                     ...
                                     */
                                    $class_file = apply_filters( "seedredux-validateclass-load", self::$_dir . "inc/validation/{$field['validate']}/validation_{$field['validate']}.php", $validate ); // REMOVE LATER

                                    /**
                                     * filter 'seedredux/validate/{opt_name}/class/{field.validate}'
                                     *
                                     * @param        string                validation class file path
                                     * @param string $class_file           validation class file path
                                     */
                                    $class_file = apply_filters( "seedredux/validate/{$this->args['opt_name']}/class/{$field['validate']}", self::$_dir . "inc/validation/{$field['validate']}/validation_{$field['validate']}.php", $class_file );
            ...
            public function section_menu( $k, $section, $suffix = "", $sections = array() ) {
                ...
                if ( isset ( $section['type'] ) && $section['type'] == "divide" ) {
                    ...
                    $string .= '<li id="' . $k . $suffix . '_section_group_li" class="seedredux-group-tab-link-li' . $hide_section . $section['class'] . $subsectionsClass . '">';
                    $string .= '<a href="javascript:void(0);" id="' . $k . $suffix . '_section_group_li_a" class="seedredux-group-tab-link-a" data-key="' . $k . '" data-rel="' . $k . $suffix . '">' . $extra_icon . $icon . '<span class="group_title">' . $section['title'] . '</span></a>';
                    ...
                    if ( $subsections ) {
                        ...
                        while ( $doLoop ) {
                            ...
                            if ( count( $sections ) < $nextK || ! isset ( $sections[ $nextK ] ) || ! isset ( $sections[ $nextK ]['subsection'] ) || $sections[ $nextK ]['subsection'] != true ) {
                                ...
                                $string .= '<li id="' . $nextK . $suffix . '_section_group_li" class="seedredux-group-tab-link-li ' . $hide_sub . $section[ $nextK ]['class'] . ( $icon ? ' hasIcon' : '' ) . '">';
                                $string .= '<a href="javascript:void(0);" id="' . $nextK . $suffix . '_section_group_li_a" class="seedredux-group-tab-link-a" data-key="' . $nextK . '" data-rel="' . $nextK . $suffix . '">' . $icon . '<span class="group_title">' . $sections[ $nextK ]['title'] . '</span></a>';
                                $string .= '</li>';
                            }
                        }
...
// section_menu()
            ...
            public function generate_panel() {
                include_once( 'core/panel.php' );
                $panel = new seedreduxCorePanel ( $this );
                $panel->init();
                $this->set_transients();
            }
            ...
            public function _section_desc( $section ) {
                ...
                if ( isset ( $this->sections[ $id ]['desc'] ) && ! empty ( $this->sections[ $id ]['desc'] ) ) {
                    echo '<div class="seedredux-section-desc">' . $this->sections[ $id ]['desc'] . '</div>';
                }
            }
            ...
            public function _field_input( $field, $v = null ) {
                ...
                if ( isset ( $field['callback'] ) && ( is_callable( $field['callback'] ) || ( is_string( $field['callback'] ) && function_exists( $field['callback'] ) ) ) ) {
                    ...
                    /**
                     * action 'seedredux-before-field-{opt_name}'
                     *
                     * @deprecated
                     *
                     ...
                     */
                    do_action( "seedredux-before-field-{$this->args['opt_name']}", $field, $value ); // REMOVE

                    /**
                     * action 'seedredux/field/{opt_name}/{field.type}/callback/before'
                     *
                     * @param array  $field field data
                     * @param string $value field.id
                     */
                    do_action( "seedredux/field/{$this->args['opt_name']}/{$field['type']}/callback/before", $field, $value );

                    /**
                     * action 'seedredux/field/{opt_name}/callback/before'
                     *
                     * @param array  $field field data
                     * @param string $value field.id
                     */
                    do_action( "seedredux/field/{$this->args['opt_name']}/callback/before", $field, $value );

                    call_user_func( $field['callback'], $field, $value );


                    /**
                     * action 'seedredux-after-field-{opt_name}'
                     *
                     * @deprecated
                     *
                     * @param array  $field field data
                     * @param string $value field.id
                     */
                    do_action( "seedredux-after-field-{$this->args['opt_name']}", $field, $value ); // REMOVE

                    /**
                     * action 'seedredux/field/{opt_name}/{field.type}/callback/after'
                     *
                     * @param array  $field field data
                     * @param string $value field.id
                     */
                    do_action( "seedredux/field/{$this->args['opt_name']}/{$field['type']}/callback/after", $field, $value );

                    /**
                     * action 'seedredux/field/{opt_name}/callback/after'
                     *
                     * @param array  $field field data
                     * @param string $value field.id
                     */
                    do_action( "seedredux/field/{$this->args['opt_name']}/callback/after", $field, $value );
                ...
                if ( isset ( $field['type'] ) ) {
                    ...
                    if ( ! class_exists( $field_class ) ) {
//                    $class_file = apply_filters( 'seedredux/field/class/'.$field['type'], self::$_dir . 'inc/fields/' . $field['type'] . '/field_' . $field['type'] . '.php', $field ); // REMOVE
                        /**
                         * filter 'seedredux/{opt_name}/field/class/{field.type}'
                         *
                    ...
                    if ( class_exists( $field_class ) ) {
                        ...
                        /**
                         * action 'seedredux/field/{opt_name}/{field.type}/render/before'
                         *
                         * @param array  $field field data
                         * @param string $value field id
                         */
                        do_action( "seedredux/field/{$this->args['opt_name']}/{$field['type']}/render/before", $field, $value );

                        /**
                         * action 'seedredux/field/{$this->args['opt_name']}/render/before'
                         *
                         * @param array  $field field data
                         * @param string $value field id
                         */
                        do_action( "seedredux/field/{$this->args['opt_name']}/render/before", $field, $value );
                     ...
                     */
                        ...
                        /**
                         * filter 'seedredux-field-{opt_name}'
                         *
                         * @deprecated
                         *
                         ...
                         * @param array $field        field data
                         */
                        $_render = apply_filters( "seedredux-field-{$this->args['opt_name']}", ob_get_contents(), $field ); // REMOVE

                        /**
                         * filter 'seedredux/field/{opt_name}/{field.type}/render/after'
                         *
                         * @param       string        rendered field markup
                         * @param array $field        field data
                         */
                        $_render = apply_filters( "seedredux/field/{$this->args['opt_name']}/{$field['type']}/render/after", $_render, $field );

                        /**
                         * filter 'seedredux/field/{opt_name}/render/after'
                         *
                         * @param       string        rendered field markup
                         * @param array $field        field data
                         */
                        $_render = apply_filters( "seedredux/field/{$this->args['opt_name']}/render/after", $_render, $field );
                        ...
                        /**
                         * action 'seedredux/field/{opt_name}/{field.type}/fieldset/before/{opt_name}'
                         *
                         * @param array  $field field data
                         * @param string $value field id
                         */
                        do_action( "seedredux/field/{$this->args['opt_name']}/{$field['type']}/fieldset/before/{$this->args['opt_name']}", $field, $value );

                        /**
                         * action 'seedredux/field/{opt_name}/fieldset/before/{opt_name}'
                         *
                         * @param array  $field field data
                         * @param string $value field id
                         */
                        do_action( "seedredux/field/{$this->args['opt_name']}/fieldset/before/{$this->args['opt_name']}", $field, $value );
                        ...
                        if ( isset( $field['full_width'] ) && $field['full_width'] == true ) {
                            $class_string .= "seedredux_remove_th";
                        }

                        echo '<fieldset id="' . $this->args['opt_name'] . '-' . $field['id'] . '" class="' . $hidden . 'seedredux-field-container seedredux-field seedredux-field-init seedredux-container-' . $field['type'] . ' ' . $class_string . '" data-id="' . $field['id'] . '" ' . $data_string . ' data-type="' . $field['type'] . '">';
                        //}
                        ...
                        /**
                         * action 'seedredux/field/{opt_name}/{field.type}/fieldset/after/{opt_name}'
                         *
                         * @param array  $field field data
                         * @param string $value field id
                         */
                        do_action( "seedredux/field/{$this->args['opt_name']}/{$field['type']}/fieldset/after/{$this->args['opt_name']}", $field, $value );

                        /**
                         * action 'seedredux/field/{opt_name}/fieldset/after/{opt_name}'
                         *
                         * @param array  $field field data
                         * @param string $value field id
                         */
                        do_action( "seedredux/field/{$this->args['opt_name']}/fieldset/after/{$this->args['opt_name']}", $field, $value );
                    }
                }
            }
// _field_input()
            ...
            public function _can_output_css( $field ) {
                ...
                $field = apply_filters( "seedredux/field/{$this->args['opt_name']}/_can_output_css", $field );
