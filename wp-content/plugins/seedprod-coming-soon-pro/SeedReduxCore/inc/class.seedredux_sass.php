...
if (!class_exists('seedreduxSassCompiler')) {
    class seedreduxSassCompiler {
        ...
        private static function css_to_page($opt_name, $css) {
            echo '<style type="text/css" id="seedredux-' . $opt_name . '">' . $css . '</style>';
        }
...
if (!function_exists ( 'seedredux_enqueue_style')) {
    /**
     ...
     * @return      void
     */
    function seedredux_enqueue_style ($parent, $handle, $css_src, $scss_dir, $deps = array(), $ver = '', $media = false){
...
//echo $filename . '<br>';
                $scss_dir = SeedRedux_Helpers::cleanFilePath($scss_dir);
                $scss_dir = untrailingslashit($scss_dir);
                
                $is_diff = seedreduxSassCompiler::is_scss_newer($scss_dir, $filename);
                
                if ($is_diff) {
                    seedreduxSassCompiler::compile_single_field($parent, $scss_dir, $filename);
                    seedreduxSassCompiler::$_do_compile = true;
                }
                
                seedreduxSassCompiler::add_path($scss_dir);
                seedreduxSassCompiler::add_import('@import "' . $filename . '.scss"');
            //}
        } else {
            wp_enqueue_style(
                $handle,
