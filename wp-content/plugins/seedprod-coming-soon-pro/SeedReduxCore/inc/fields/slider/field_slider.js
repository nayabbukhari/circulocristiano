...
(function( $ ) {
    ...
    seedredux.field_objects.slider = seedredux.field_objects.slider || {};
    ...
    seedredux.field_objects.slider.init = function( selector ) {

        if ( !selector ) {
            selector = $( document ).find( ".seedredux-group-tab:visible" ).find( '.seedredux-container-slider:visible' );
        }
        ...
        $( selector ).each(
            function() {
                ...
                el.find( 'div.seedredux-slider-container' ).each(
                    function() {
                        ...
                        if ( displayValue == DISPLAY_TEXT ) {
                            defClassOne = el.find( '.seedredux-slider-input-one-' + mainID );
                            defClassTwo = el.find( '.seedredux-slider-input-two-' + mainID );

                            inputOne = defClassOne;
                            inputTwo = defClassTwo;
                        } else if ( displayValue == DISPLAY_SELECT ) {
                            defClassOne = el.find( '.seedredux-slider-select-one-' + mainID );
                            defClassTwo = el.find( '.seedredux-slider-select-two-' + mainID );

                            seedredux.field_objects.slider.loadSelect( defClassOne, minVal, maxVal, resVal, stepVal );

                            if ( handles === 2 ) {
                                seedredux.field_objects.slider.loadSelect( defClassTwo, minVal, maxVal, resVal, stepVal );
                            }

                        } else if ( displayValue == DISPLAY_LABEL ) {
                            defClassOne = el.find( '#seedredux-slider-label-one-' + mainID );
                            defClassTwo = el.find( '#seedredux-slider-label-two-' + mainID );
                        } else if ( displayValue == DISPLAY_NONE ) {
                            defClassOne = el.find( '.seedredux-slider-value-one-' + mainID );
                            defClassTwo = el.find( '.seedredux-slider-value-two-' + mainID );
                        }
                                ...
                                slide: function() {
                                    if ( displayValue == DISPLAY_LABEL ) {
                                        if ( handles === 2 ) {
                                            ...
                                            el.find( 'input.seedredux-slider-value-one-' + mainID ).attr(
                                                'value', inpSliderVal[0]
                                            );
                                            el.find( 'input.seedredux-slider-value-two-' + mainID ).attr(
                                    ...
                                    if ( displayValue == DISPLAY_SELECT ) {
                                        if ( handles === 2 ) {
                                            el.find( '.seedredux-slider-select-one' ).select2( 'val', slider.val()[0] );
                                            el.find( '.seedredux-slider-select-two' ).select2( 'val', slider.val()[1] );
                                        } else {
                                            el.find( '.seedredux-slider-select-one' ).select2( 'val', slider.val() );
                                        }
                                    }

                                    seedredux_change( $( this ).parents( '.seedredux-field-container:first' ).find( 'input' ) );
                                }
                            }
                        );
                ...
                el.find( 'select.seedredux-slider-select-one, select.seedredux-slider-select-two' ).select2( default_params );
    ...
    seedredux.field_objects.slider.isFloat = function( mixed_var ) {
        return +mixed_var === mixed_var && (!(isFinite( mixed_var ))) || Boolean( (mixed_var % 1) );
    };

    // Return number of integers after the decimal point.
    seedredux.field_objects.slider.decimalCount = function( res ) {
        var q = res.toString().split( '.' );
        return q[1].length;
    };

    seedredux.field_objects.slider.loadSelect = function( myClass, min, max, res, step ) {
        ...
        for ( var i = min; i <= max; i = i + res ) {
            ...
            var n = i;
            if ( seedredux.field_objects.slider.isFloat( res ) ) {
                var decCount = seedredux.field_objects.slider.decimalCount( res );
