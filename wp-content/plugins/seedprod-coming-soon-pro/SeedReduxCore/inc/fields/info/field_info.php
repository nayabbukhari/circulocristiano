...
// Don't duplicate me!
    if ( ! class_exists( 'SeedReduxFramework_info' ) ) {

        /**
         * Main SeedReduxFramework_info class
         *
         * @since       1.0.0
         */
        class SeedReduxFramework_info {
            ...
            public function render() {
                ...
                if ( empty( $this->field['raw_html'] ) ) {
                    if ( $this->field['notice'] == true ) {
                        $this->field['class'] .= ' seedredux-notice-field';
                    } else {
                        $this->field['class'] .= ' seedredux-info-field';
                    }



                    $this->field['style'] = 'seedredux-' . $this->field['style'] . ' ';
                }

                $indent = ( isset( $this->field['sectionIndent'] ) && $this->field['sectionIndent'] ) ? ' form-table-section-indented' : '';

                echo '</td></tr></table><div id="info-' . $this->field['id'] . '" class="' . $this->field['style'] . $this->field['class'] . ' seedredux-field-' . $this->field['type'] . $indent . '"'.( !empty($this->field['color']) ? ' style="'.$this->field['color'].'"' : '' ).'>';
                ...
                if ( ! empty( $this->field['raw_html'] ) && $this->field['raw_html'] ) {
                    ...
                    if ( isset( $this->field['icon'] ) && ! empty( $this->field['icon'] ) && $this->field['icon'] !== true ) {
                        echo '<p class="seedredux-info-icon"><i class="' . $this->field['icon'] . ' icon-large"></i></p>';
                    }
                    ...
                    if ( ! empty( $this->field['title'] ) || ! empty( $this->field['desc'] ) ) {
                        echo '<p class="seedredux-info-desc">' . $this->field['title'] . $this->field['desc'] . '</p>';
                    }
                }
            ...
            public function enqueue() {
                if ($this->parent->args['dev_mode']) {
                    wp_enqueue_style(
                        'seedredux-field-info-css',
                        SeedReduxFramework::$_url . 'inc/fields/info/field_info.css',
                        array(),
                        time(),
                        'all'
                    );
