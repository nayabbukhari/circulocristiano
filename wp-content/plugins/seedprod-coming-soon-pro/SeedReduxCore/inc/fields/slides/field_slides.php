...
// Don't duplicate me!
if ( !class_exists ( 'SeedReduxFramework_slides' ) ) {

    /**
     * Main SeedReduxFramework_slides class
     *
     * @since       1.0.0
     */
    class SeedReduxFramework_slides {
        ...
        public function render () {
            ...
            $defaults = array(
                ...
                'content_title' => __ ( 'Slide', 'seedredux-framework' )
            );

            $this->field = wp_parse_args ( $this->field, $defaults );

            echo '<div class="seedredux-slides-accordion" data-new-content-title="' . esc_attr ( sprintf ( __ ( 'New %s', 'seedredux-framework' ), $this->field[ 'content_title' ] ) ) . '">';
            ...
            if ( isset ( $this->value ) && is_array ( $this->value ) && !empty ( $this->value ) ) {
                ...
                foreach ( $slides as $slide ) {
                    ...
                    echo '<div class="seedredux-slides-accordion-group"><fieldset class="seedredux-field" data-id="' . $this->field[ 'id' ] . '"><h3><span class="seedredux-slides-header">' . $slide[ 'title' ] . '</span></h3><div>';
                    ...
                    echo '<img class="seedredux-slides-image" id="image_image_id_' . $x . '" src="' . $slide[ 'thumb' ] . '" alt="" target="_blank" rel="external" />';
                    echo '</a>';
                    echo '</div>';

                    echo '<div class="seedredux_slides_add_remove">';

                    echo '<span class="button media_upload_button" id="add_' . $x . '">' . __ ( 'Upload', 'seedredux-framework' ) . '</span>';
                    ...
                    echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $slide[ 'attachment_id' ] . '">' . __ ( 'Remove', 'seedredux-framework' ) . '</span>';

                    echo '</div>' . "\n";

                    echo '<ul id="' . $this->field[ 'id' ] . '-ul" class="seedredux-slides-list">';
                    ...
                    $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'title' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'title' ] ) : __ ( 'Title', 'seedredux-framework' );
                    ...
                    if ( $this->field[ 'show' ][ 'description' ] ) {
                        $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'description' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'description' ] ) : __ ( 'Description', 'seedredux-framework' );
                    ...
                    $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'url' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'url' ] ) : __ ( 'URL', 'seedredux-framework' );
                    ...
                    echo '<li><a href="javascript:void(0);" class="button deletion seedredux-slides-remove">' . __ ( 'Delete', 'seedredux-framework' ) . '</a></li>';
            ...
            if ( $x == 0 ) {
                echo '<div class="seedredux-slides-accordion-group"><fieldset class="seedredux-field" data-id="' . $this->field[ 'id' ] . '"><h3><span class="seedredux-slides-header">' . esc_attr ( sprintf ( __ ( 'New %s', 'seedredux-framework' ), $this->field[ 'content_title' ] ) ) . '</span></h3><div>';

                $hide = ' hide';

                echo '<div class="screenshot' . $hide . '">';
                echo '<a class="of-uploaded-image" href="">';
                echo '<img class="seedredux-slides-image" id="image_image_id_' . $x . '" src="" alt="" target="_blank" rel="external" />';
                echo '</a>';
                echo '</div>';
                ...
                echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $this->parent->args[ 'opt_name' ] . '[' . $this->field[ 'id' ] . '][attachment_id]">' . __ ( 'Remove', 'seedredux-framework' ) . '</span>';
            ...
            echo '</div><a href="javascript:void(0);" class="button seedredux-slides-add button-primary" rel-id="' . $this->field[ 'id' ] . '-ul" rel-name="' . $this->field[ 'name' ] . '[title][]' . $this->field['name_suffix'] .'">' . sprintf ( __ ( 'Add %s', 'seedredux-framework' ), $this->field[ 'content_title' ] ) . '</a><br/>';
        }
        ...
        public function enqueue () {
            ...
            if ($this->parent->args['dev_mode']){
                ...
                wp_enqueue_style (
                    'seedredux-field-slides-css', 
                    SeedReduxFramework::$_url . 'inc/fields/slides/field_slides.css', 
                    array(),
                    time (), 
                    'all'
                );
            ...
            wp_enqueue_script(
                'seedredux-field-media-js',
                SeedReduxFramework::$_url . 'assets/js/media/media' . SeedRedux_Functions::isMin() . '.js',
            ...
            wp_enqueue_script (
                'seedredux-field-slides-js', 
                SeedReduxFramework::$_url . 'inc/fields/slides/field_slides' . SeedRedux_Functions::isMin () . '.js', 
                array( 'jquery', 'jquery-ui-core', 'jquery-ui-accordion', 'jquery-ui-sortable', 'seedredux-field-media-js' ),
                time (), 
                true
            );
        }
