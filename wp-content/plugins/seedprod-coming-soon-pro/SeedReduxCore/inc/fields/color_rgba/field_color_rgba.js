(function($){
    ...
    seedredux.field_objects.color_rgba          = seedredux.field_objects.color_rgba || {};
    seedredux.field_objects.color_rgba.fieldID  = '';

    seedredux.field_objects.color_rgba.hexToRGBA = function( hex, alpha ) {
        var result;
    ...
    seedredux.field_objects.color_rgba.init = function( selector ) {
        if ( !selector ) {
            selector = $( document ).find( ".seedredux-group-tab:visible" ).find( '.seedredux-container-color_rgba:visible' );
        }
        ...
        $( selector ).each(
            function() {
                ...
                seedredux.field_objects.color_rgba.modInit(el);
                seedredux.field_objects.color_rgba.initColorPicker(el);
            });
    };

    seedredux.field_objects.color_rgba.modInit = function(el) {

        seedredux.field_objects.color_rgba.fieldID    = el.find('.seedredux-color_rgba-container').data('id');

    };

    // Initialize colour picker
    seedredux.field_objects.color_rgba.initColorPicker = function(el){

        // Get field ID
        var field_id    = seedredux.field_objects.color_rgba.fieldID;

        // Get the color scheme container
        var colorpickerInput = el.find('.seedredux-color-rgba');
        ...
        var container   = el.find('.seedredux-color-rgba-container');
        ...
        colorpickerInput.spectrum({
            ...
            className:              'seedredux-color-rgba',
            ...
            localStorageKey:        'seedredux.color-rgba.' + field_id,
            palette:                palette,
            inputText:              inputText,
            ...
            change: function(color) {
                ...
                if (colorVal != 'transparent') {
                    rgbaVal     = seedredux.field_objects.color_rgba.hexToRGBA(colorVal, alphaVal);
                } else {
                    rgbaVal     = 'transparent';
                }
                ...
                seedredux_change(el.find('.seedredux-color-rgba-container'));
            }
        });
    };
})(jQuery);