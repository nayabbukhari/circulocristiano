...
// Don't duplicate me!
if( !class_exists( 'SeedReduxFramework_color_rgba' ) ) {

    /**
     * Main SeedReduxFramework_color_rgba class
     *
     * @since       1.0.0
     */
    class SeedReduxFramework_color_rgba {
       ...
       */
        ...
        public function render() {
            ...
            echo '<div 
                      class="seedredux-color-rgba-container' . $this->field['class'] . '" 
                      data-id="'                        . $field_id . '"
            ...
            if ('' == $this->value['color'] || 'transparent' == $this->value['color']) {
                ...
                $color = SeedRedux_Helpers::hex2rgba($this->value['color'], $this->value['alpha']);
            }            

            if ($this->value['rgba'] == ''){
                $this->value['rgba'] = SeedRedux_Helpers::hex2rgba($this->value['color'], $this->value['alpha']);
            }
            
            echo '<input
                        name="' . $opt_name . '[' . $field_id . '][color]"
                        id="' . $field_id . '-color"
                        class="seedredux-color-rgba"
                        type="text"
        ...
        public function enqueue() {
            ...
            wp_enqueue_script(
                'seedredux-spectrum-js', 
                SeedReduxFramework::$_url . 'assets/js/vendor/spectrum/seedredux-spectrum' . $min . '.js', 
                array('jquery'), 
                time(), 
                true
            );
            ...
            // Field dependent JS
            wp_enqueue_script(
                'seedredux-field-color-rgba-js', 
                SeedReduxFramework::$_url . 'inc/fields/color_rgba/field_color_rgba' . SeedRedux_Functions::isMin() . '.js',
                array('jquery', 'seedredux-spectrum-js'), 
                time(), 
                true
            );
            
            // Spectrum CSS
            wp_enqueue_style('seedredux-spectrum-css');
            
            if ($this->parent->args['dev_mode']) {
                wp_enqueue_style(
                    'seedredux-field-color-rgba-css',
                    SeedReduxFramework::$_url . 'inc/fields/color_rgba/field_color_rgba.css',
                    array(),
                    time(),
                    'all'
                );
        ...
        public function output() {
            if (!empty($this->value)) {
                ...
                if ( ! empty( $this->field['output'] ) && is_array( $this->field['output'] ) ) {
                    $css = SeedRedux_Functions::parseCSS( $this->field['output'], $style, $color_val );
                    $this->parent->outputCSS .= $css;
                }

                if ( ! empty( $this->field['compiler'] ) && is_array( $this->field['compiler'] ) ) {
                    $css = SeedRedux_Functions::parseCSS( $this->field['compiler'], $style, $color_val );
