...
function	seed_cspv4_norm_str($string) {
	return	trim(strtolower(
		str_replace('.','',$string)));
	}

function	seed_cspv4_in_array_norm($needle,$haystack) {
	return	in_array(seed_cspv4_norm_str($needle),$haystack);
	}

function	seed_cspv4_parse_name($fullname) {
	...
	switch($n_pieces) {
		case	1:	// array(title first middles last suffix)
			...
			for($i = 0; $i < $n_subp; $i++) {
				...
				if($i == 0 && seed_cspv4_in_array_norm($curr,$titles)) {
					$out['title']	=	$curr;
					continue;
					}
				...
				if($i == $n_subp-2 && $next && seed_cspv4_in_array_norm($next,$suffices)) {
					if($out['last']) {
						$out['last']	.=	" $curr";
						}
					else {
				...
				if(seed_cspv4_in_array_norm($curr,$prefices)) {
					if($out['last']) {
						$out['last']	.=	" $curr";
						}
					else {
			...
			break;
		case	2:
				switch(seed_cspv4_in_array_norm($pieces[1],$suffices)) {
					...
					case	FALSE: // array(last,title first middles suffix)
						...
						if($i == $n_subp-2 && $next &&
							seed_cspv4_in_array_norm($next,$suffices)) {
							if($out['middle']) {
								$out['middle']	.=	" $curr";
								}
							else {
						...
						if($i == $n_subp-1 && seed_cspv4_in_array_norm($curr,$suffices)) {
							$out['suffix']		=	$curr;
							continue;
							}
